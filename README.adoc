...
TODO if I run concat on an argument value, why do I not get the content with expanded elements
TODO fix confusion regarding outer and inner byte offsets

function concat(tbl)
  local cat = ""
  for c = 1,#tbl do
    cat = cat .. tostring(tbl[c])
  end
  return cat
end
...
* ``Litua.modify_initial_string = function (hook, filepath) […] end`` where ``hook`` takes the source code as a string, the ``filepath`` of its file and returns a string
