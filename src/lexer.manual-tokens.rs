...
impl<'l> LexingIterator<'l> {
    fn push_scope(&mut self, sc: LexingScope, byte_offset: usize) {
        ...
        //println!("push for {:?}", sc);
        self.next_tokens.push_back(match sc {
            CallInContent | CallInArgumentValue => Token::BeginFunction(byte_offset),
            ArgumentValueInCall => Token::BeginArgValue(byte_offset),
            ContentInCall => Token::StartContent(byte_offset),
        });
        self.stack.push(sc);
    }
    ...
    fn pop_scope(&mut self, byte_offset: usize) {
        ...
        println!("pop {:?}", self.stack.last());
        match self.stack.pop() {
            Some(CallInContent | CallInArgumentValue) => {
                self.next_tokens.push_back(Token::EndFunction(byte_offset));
            },
            Some(ArgumentValueInCall) => {
    ...
    fn progress(&mut self) -> Option<Token> {
        ...
        match self.state {
            ReadingContent => {
                //println!("ReadingContent {:?}", self.stack);
                match chr {
                    OPEN_CALL => {
                        if self.token_start != 0 {
                            self.next_tokens.push_back(Token::StartContent(self.token_start));
                            self.next_tokens.push_back(Token::EndContent(byte_offset));
                        }
            ...
            ReadingCallName => {
                match chr {
                    CLOSE_CALL => {
                        let tok = Token::Call(self.token_start..byte_offset);
                        self.pop_scope(byte_offset);
                        return Some(tok);
                    },
                    c if c.is_whitespace() => {
                        let arg = Token::Call(self.token_start..byte_offset);
...
impl Token {
    pub fn format_with_src(&self, source: &str) -> String {
        ...
        match self {
            ...
            StartContent(pos) => match source.get(*pos..) {
                Some(substr) => format!("StartContent @ {}({:?})", pos, substr.chars().take(1).collect::<String>()),
                None => format!("StartContent @ {}", pos),
            },
